...
export class MSSQLBalanceRepository implements BalanceRepository {
  public async find(id: number): Promise<Balance | null> {
    const pool = await connector
    const result =
      await pool.query`SELECT * FROM balance WHERE id = ${id}`
  ...
  public async findByUserId(userId: number): Promise<Balance | null> {
    const pool = await connector
    const result =
      await pool.query`SELECT * FROM balance WHERE user_id = ${userId}`
  ...
  public async all(): Promise<Balance[]> {
    const pool = await connector
    const result =
      await pool.query`SELECT * FROM balance ORDER BY id DESC`
  ...
  public async store(entry: Balance): Promise<void> {
    ...
    await pool.query`INSERT INTO balance(user_id, amount, created_at)
  ...
  public async remove(id: number): Promise<void> {
    ...
    await pool.query`DELETE FROM balance WHERE id = ${id}`
  }
}
